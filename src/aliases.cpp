...
void Aliases::addAlias (QString alias, QString command)
{
	...
	/* Add alias */
	fp = popen ((QString("echo \"alias ")+alias_buf+QString("=")+command_buf+QString("\" >> /home/$(whoami)/.bashrc")).toLocal8Bit(),"w");
...
void Aliases::removeAlias (QString alias)
{
	...
	/* Remove alias */
	fp = popen ((QString("sed -i '/")+alias_buf+QString("/d' ~/.bashrc")).toLocal8Bit(),"w");
...
void Aliases::readAliasesList (void)
{
	...
	fp = popen ("cat /home/$(whoami)/.bashrc | grep alias | sed 's/^ *//' | sed '/^#/d' | sed -n '/^alias/p' | sed -e ':1' -e 's/^alias //;t1' > /tmp/yakala.alias ","r");
